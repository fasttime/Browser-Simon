...
m()},3E3);a.b(function(){c(d)})}var l=new Map,n,r;(function(){var a=URL.createObjectURL(new Blob(['let idMap=new Map;onmessage=event=>{let notify=()=>postMessage({id});let startDelay=callback=>idMap.set(id,setTimeout(()=>{callback();notify()},data.delay));let data=event.data,id=data.id;switch(data.action){case"START":startDelay(()=>idMap.delete(id));break;case"REPEAT":startDelay(()=>idMap.set(id,setInterval(notify,data.interval)));break;case"STOP":let nativeId=idMap.get(id);idMap.delete(id);clearTimeout(nativeId);break;}}']));
...
margin:"3px",width:"92px",height:"92px"});art.a(".start:active",{animation:"start 1.5s infinite","border-left-color":"#AAA","border-right-color":"#DDD","border-top-color":"#AAA","border-bottom-color":"#DDD"});art.a(".start,.startLayer",{"border-radius":"100%","border-style":"solid","border-left-color":"#DDD","border-right-color":"#AAA","border-top-color":"#DDD","border-bottom-color":"#AAA","box-sizing":"border-box"});z=art("SPAN","\u2014");v=art("H2",{style:{textAlign:"center"}},"Hello");d=art("FOOTER",
{style:{display:"table",margin:"18px 0",tableLayout:"fixed",whiteSpace:"nowrap",width:"100%"}},art("H2",{style:{overflowX:"auto"}},"Round: ",z),v);art.a("h2",{display:"table-cell",font:"24px Verdana"});art(document.body,{style:{color:"#33A",WebkitTapHighlightColor:"transparent",u:"none",MozUserSelect:"none",WebkitUserSelect:"none"}},art("DIV",{style:{margin:"0 auto",width:"300px"}},a,b,d))}))})();
